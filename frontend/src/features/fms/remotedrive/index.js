...
import useIntentLoop from "./useIntentLoop"
...
function TeleopPanel() {
    ...
    } = useKeyboardControl(telemetryRef);

    const { cameraUrl, telemetry, clientId } = useIntentLoop(
    ...
    return (
        <div className="w-full h-[calc(100vh-100px)] grid grid-cols-12 gap-4 bg-base-100 overflow-hidden">
            ...
            {/* Control Panel — 3-row grid with fixed 1:3:6 proportions.
                Replaces the previous flex+overflow-y-auto stack; every
                descendant is grid-based with explicit cell sizing, so
                child content never reshapes sibling panels' heights. */}
            <div className="col-span-4 lg:col-span-3 h-full grid grid-rows-[3fr_7fr_10fr] gap-4 overflow-hidden pr-2 pb-2">
                {/* Vehicle Selection (row 1, 10%) — grid: header row on top,
                    body row with select + button side-by-side. */}
                ...
                {/* Key Controls (row 2, 30%) — grid: upper half = WASD diamond
                    + label, lower half = one-shot legend grid. */}
                <div className="bg-gradient-to-br from-base-200 to-base-300 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-lg border border-base-300/50 grid grid-rows-[auto_1fr] gap-2 h-full overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1 w-full text-xs font-medium text-base-content/80 content-center">
                        ...
                        <div className="grid grid-cols-[auto_1fr] gap-2 items-center min-w-0">
                            <KeyBadge keyName="space" label="SPC/S" width="w-12" onPressStart={() => pressKey('space')} onPressEnd={() => releaseKey('space')} />
                            <span className="truncate">E-Brake</span>
                        </div>
                    </div>
                </div>

                {/* Cockpit (row 3, 60%) — 2×2 grid. TL wheel, TR label/hint,
                    BL brake, BR gas. `h-full` so the 2×2 fills its row. */}
                <div className="bg-gradient-to-br from-base-200 to-base-300 px-10 py-4 rounded-3xl shadow-xl border border-base-300/50 grid grid-cols-2 grid-rows-2 gap-4 h-full overflow-hidden">
                    ...
                    {/* BL — Brake. Also lights up on SPACE because the C++
                        input parser folds `space` into `brake` (SPACE additionally
                        raises emergency_stop, which toggles STOP mode — but the
                        brake-pedal signal is the same either way). */}
                    <button
                        className={`w-full h-full rounded-3xl transition-all duration-200 select-none border backdrop-blur-md flex items-center justify-center ${(pressedKeys.includes('s') || pressedKeys.includes('space')) ? 'bg-red-500/20 border-red-500/50 scale-95' : 'bg-base-100/40 border-base-content/10 shadow-xl'}`}
                    ...
                    >
                        <span className={`font-medium tracking-widest text-xs uppercase ${(pressedKeys.includes('s') || pressedKeys.includes('space')) ? 'text-red-500' : 'text-base-content/60'}`}>Brake</span>
                    </button>
                    {/* BR — Gas */}
                    <button
