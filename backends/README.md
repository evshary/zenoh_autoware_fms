...
`dev.sh up`, `dev.sh down`, and `dev.sh bootstrap` source the chosen
...
Before `dev.sh up` runs, the backend must have produced:
...
|---|---|---|
| `backend_check_runtime_prereqs` | `dev.sh up` | bail early if §1/§2 missing |
| `backend_check_bootstrap_prereqs` | `dev.sh bootstrap` | bail early on first-build inputs |
| `backend_bootstrap` | `dev.sh bootstrap` | one-time build of all artifacts |
| `backend_seed_frontend_assets` | both | mirror map etc. into `frontend/public/` |
| `backend_seed_custom_configs` | `dev.sh up` | apply per-deploy overrides (no-op if none) |
| `backend_export_runtime_flags` | `dev.sh up` | export backend-specific runtime env (e.g. `USE_BRIDGE_ROS2DDS`); `REACT_APP_*` come from `env.sh` via `just run` |
| `backend_start_sim` | `dev.sh up` | start sim (no-op for real cars) |
| `backend_start_bridge` | `dev.sh up` | start sim↔ROS bridge (no-op for real cars) |
| `backend_start_autoware` | `dev.sh up` | start Autoware bringup |
| `backend_wait_ready` | `dev.sh up` (optional) | wait for `is_remote_mode_available: true` |
| `backend_exec_in_ros [-d] <cmd>` | `dev.sh up` | run `cmd` in the backend's ROS shell; `-d` = detached |
| `backend_stop` | `dev.sh up`, `dev.sh down` | stop sim/bridge/autoware |
...
`dev.sh up` (precondition: `backend_check_runtime_prereqs`):
...
Numbering is generated by `dev.sh::run_steps` from a `STEPS`
array — adding/removing a step renumbers automatically.

`dev.sh down`: stop FMS pieces → `backend_stop`.
